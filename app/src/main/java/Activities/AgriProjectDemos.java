...
import Adapters.AgriDemoAdapter;
import Adapters.OfficeDemosAdapter;

public class AgriProjectDemos extends AppCompatActivity {
    FirebaseStorage storage;
    ArrayList<StorageReference> imageURLs = new ArrayList<StorageReference>();
    AgriDemoAdapter adapter;
    RecyclerView recycle_view;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_agri_project_demos);
        storage = FirebaseStorage.getInstance();
        setupStorage();
    }
    public void set_up_recycle_view(){
        ArrayList<String> imageURLs1 = new ArrayList<>();
        adapter = new AgriDemoAdapter(this, imageURLs1);
        GridLayoutManager gridLayoutManager = new GridLayoutManager(this, 2);
        recycle_view = findViewById(R.id.recycle_view_agri);
    ...
    private void setupStorage() {
        StorageReference listRef = storage.getReference().child("agri");
        listRef.listAll()
